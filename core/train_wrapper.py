...
def train_wrapper(args, model, writer, save_prefix = '', kth_specific_category = None):
    ...
    train_input_handle, test_input_handle = datasets_factory.data_provider(
        args.dataset_name, args.train_data_paths, args.valid_data_paths, args.H5_path, args.GT_path, args.batch_size, args.img_width,
    ...
    for itr in range(1, args.max_iterations + 1):
        ...
        ims, audios, gts, train_category = train_input_handle.get_batch()
        ...
        if category>0:
            if args.is_replay:
                if itr%args.replay_interval==0:
                    ...
                    loss = trainer.CPL_train(model,pre_model, ims, audios, gts, real_input_flag, args, itr,replay_cat,is_replay=True)
        loss = trainer.CPL_train(model,pre_model, ims,  audios, gts, real_input_flag, args, itr,category)
        if itr % args.display_interval == 0:
            writer.add_scalar("metrics/loss", loss, global_step=itr)
