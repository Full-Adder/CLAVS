...
class Model(object):
    def __init__(self, configs):
        ...
        if configs.model_name == 'CPL':
            self.prediction_optimizer = Adam(
                itertools.chain(self.network.encoder.parameters(), self.network.decoder.parameters(),
                                self.network.prior.parameters(), self.network.embed_class.parameters(),
                                self.network.embed_data.parameters()), lr=configs.lr)
            self.recon_optimizer = Adam(
                itertools.chain(self.network.head_reconstructor.parameters(), self.network.prior_fp.parameters(),
                                self.network.shared_encoder.parameters(), self.network.latent_encoder.parameters()),
                lr=configs.lr)

            param_groups = self.network.STAnet.get_parameter_groups()  # 获取 1作者定义的网络的权重 2作者定义的网罗的偏置 3resnext的权重 4resnext的偏
            self.STA_optimizer = SGD([  # 随机梯度下降
                {'params': param_groups[0], 'lr': 1/5 * configs.lr},  # 对于不同类型参数用不同的学习率
                {'params': param_groups[1], 'lr': 2/5 * configs.lr},
                {'params': param_groups[2], 'lr': 2 * configs.lr},
                {'params': param_groups[3], 'lr': 4 * configs.lr}], momentum=0.9, weight_decay=0.0005, nesterov=True)
        else:
    ...
    def CPL_train(self, pre_model, frames, audios, gts, mask, category, itr, gt_frames=None, is_replay=False):
        frames_tensor = torch.FloatTensor(frames).to(self.configs.device)
        audios = torch.FloatTensor(audios).to(self.configs.device)
        gts = torch.FloatTensor(gts).to(self.configs.device)
        mask_tensor = torch.FloatTensor(mask).to(self.configs.device)
        ...
        if is_replay:
            if itr % self.configs.replay_interval == 0:
                ...
                self.STA_optimizer.zero_grad()
                next_frames, loss, loss_pd, loss_kl, loss_cat, loss_recon, loss_STA = self.network(gen_frames_tensor, audios, gts, mask_tensor, # Todo aud gt need to reload
                                                                                         category, gt_frames, is_train=True,
                                                                                         is_replay=False)
                loss.backward()
                self.prediction_optimizer.step()
                self.recon_optimizer.step()
                self.STA_optimizer.step()

        else:
            self.prediction_optimizer.zero_grad()
            self.recon_optimizer.zero_grad()
            self.STA_optimizer.zero_grad()
            next_frames, loss, loss_pd, loss_kl, loss_cat, loss_recon, loss_STA = self.network(frames_tensor, audios, gts, mask_tensor,
                                                                                     category, is_train=True)
            loss.backward()
            self.prediction_optimizer.step()
            self.recon_optimizer.step()
            self.STA_optimizer.step()

        return loss.detach().cpu().numpy(), loss_pd.detach().cpu().numpy(), loss_kl.detach().cpu().numpy(), loss_cat.detach().cpu().numpy(), loss_recon.detach().cpu().numpy(), loss_STA.detach().cpu().numpy()

    def test(self, frames, audios, gts, mask, category):
        frames_tensor = torch.FloatTensor(frames).to(self.configs.device)
        ...
        if self.configs.model_name == 'CPL_base':
            ...
            if self.configs.is_training:
                with torch.no_grad():
                    next_frames = \
                    self.network(frames_tensor[:, :self.configs.input_length // 2],audios, gts, mask_tensor, 0, is_train=0,
                                 is_replay=False)[0]
                    next_frames = torch.cat((frames_tensor[:, :1], next_frames[:, :self.configs.input_length - 1]), 1)
                    loss = nn.MSELoss()(next_frames[:, :self.configs.input_length],
                                        frames_tensor[:, :self.configs.input_length])
                    pred_cat = 0

                    for i in range(1, self.configs.num_classes):
                        next_frames = \
                        self.network(frames_tensor[:, :self.configs.input_length // 2],audios, gts, mask_tensor, i, is_train=0,
                                     is_replay=False)[0]
                        next_frames = torch.cat((frames_tensor[:, :1], next_frames[:, :self.configs.input_length - 1]),
                                                1)
                        loss_i = nn.MSELoss()(next_frames[:, :self.configs.input_length],
                                              frames_tensor[:, :self.configs.input_length])
                        if loss_i < loss:
                            loss = loss_i
                            pred_cat = i

                    STA_img = self.network.reshape_patchback_for_STA(frames_tensor, self.configs.patch_size)
                    map0, map0_1, map1, map1_1, map2, map2_1 = self.network.STAnet(STA_img, audios)


                    next_frames, right_cat_num = self.network(frames_tensor,audios, gts, mask_tensor, pred_cat, is_train=0)
            else:
                # test_model.load(self.configs.pretrained_model)
                with torch.no_grad():
                    next_frames = \
                    self.network(frames_tensor[:, :self.configs.input_length // 2], mask_tensor, 0, is_train=0,
                                 is_replay=False)[0]
                    next_frames = torch.cat((frames_tensor[:, :1], next_frames[:, :self.configs.input_length - 1]), 1)
                    ...
                    for i in range(1, self.configs.num_classes):
                        next_frames = \
                        self.network(frames_tensor[:, :self.configs.input_length // 2], mask_tensor, i, is_train=0,
                                     is_replay=False)[0]
                        next_frames = torch.cat((frames_tensor[:, :1], next_frames[:, :self.configs.input_length - 1]),
                                                1)
            ...
            return next_frames.detach().cpu().numpy(), right_cat_num.detach().cpu().numpy(), map0.detach().cpu().numpy(), map1.detach().cpu().numpy(), map2.detach().cpu().numpy()
