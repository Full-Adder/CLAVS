...
class latent_encoder(nn.Module):
    def __init__(self,configs):
        ...
        # nn.Linear(configs.zdim+configs.num_classes,configs.zdim),
        # )
        self.fc_mu = nn.Linear(512*4*4, configs.zdim)
        self.fc_var = nn.Linear(512*4*4, configs.zdim)
...
class encoder(nn.Module):
    def __init__(self,configs):
        ...
        self.c1 = nn.Sequential(
                nn.Conv2d(self.in_channel*2*3, 256, 5, 2, padding=2),
                nn.ReLU(True),
                )
...
class decoder(nn.Module):
    def __init__(self,num_layers,num_hidden,configs):
        ...
        self.conv=nn.Conv2d(num_hidden,self.frame_channel, kernel_size=1, stride=1, padding=0, bias=False)
        cell_list=[]
