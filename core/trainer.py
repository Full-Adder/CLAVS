...
def imgtocolmap(img, cam_gt, save_path):
    cam_gt = cv2.resize(cam_gt, (128, 128))
    att = torch.from_numpy(cam_gt)
    att = F.sigmoid(att)
    att = np.rint((att-att.min()) / (att.max() + 1e-10) * 255)  # 归一化到0-255
...
def CPL_train(model, pre_model, ims, audios, gts, real_input_flag, configs, itr, category, is_replay=False):
    cost, cost_pd, cost_kl, cost_cat, loss_recon, loss_STA = model.CPL_train(pre_model, ims, audios, gts,
                                                                             real_input_flag, category, itr, is_replay)

    if configs.reverse_input:
        ims_rev = np.flip(ims, axis=1).copy()
        cost_, cost_pd_, cost_kl_, loss_recon, loss_STA = model.CPL_train(pre_model, ims_rev, real_input_flag, category,
                                                                          itr, is_replay)
        cost = (cost + cost_) / 2
        cost_pd = (cost_pd + cost_pd_) / 2
        cost_kl = (cost_kl + cost_kl_) / 2
    ...
    if itr % configs.display_interval == 0:
        print(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'itr: ' + str(itr))
        print('loss: ' + str(cost) + ', pd: ' + str(cost_pd) + ',recon' + str(loss_recon) + ', kl: ' + str(
            cost_kl) + ', STA: ' + str(loss_STA))
    return cost
...
def test(model, test_input_handle, configs, itr, category=None):
    ...
    while not test_input_handle.no_batch_left():
        ...
        test_ims, test_aud, test_gts, test_cat = test_input_handle.get_batch()

        test_dat = preprocess.reshape_patch(test_ims, configs.patch_size)
        if configs.model_name == 'CPL_base':
            img_gen, prd0, prd1, prd2 = model.test(test_dat, sample, test_cat)
        elif configs.model_name == 'CPL':
            img_gen, right_cat_num, prd0, prd1, prd2 = model.test(test_dat, test_aud, test_gts, sample, category)
        ...
        for bid in range(configs.batch_size):
            ...
            if batch_id == 0:
                if batch_id == 0:
                    ...
                    STA_re_path = os.path.join(batch_path, "STA_re")
                    if not os.path.exists(STA_re_path):
                        os.mkdir(STA_re_path)
                    ...
                    for i in range(configs.total_length):
                        ...
                        cam_path = os.path.join(STA_re_path, 'cam%02d.jpg' % i)
                        imgtocolmap(img_gt, prd[i][bid, 0], cam_path)
                        ggt_path = os.path.join(STA_re_path, 'gt%02d.jpg' % i)
                        imgtocolmap(img_gt, test_gts[bid, i, 0], ggt_path)
